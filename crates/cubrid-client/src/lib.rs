...
//! let affected = client.execute("CREATE TABLE IF NOT EXISTS t (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(100))", &[])?;
//!
//! // Execute with parameters
//! client.execute("INSERT INTO t (name) VALUES (?)", &[Value::from("hello")])?;
//!
//! // Query rows
//! let rows = client.query("SELECT id, name FROM t WHERE name = ?", &[Value::from("hello")])?;
//! for row in &rows {
//!     println!("{:?}", row);
//! }
//!
...
//! client.execute("INSERT INTO t (name) VALUES (?)", &[Value::from("tx_test")])?;
//! client.commit()?;
//!
//! // Prepared statements
//! let mut stmt = client.prepare("SELECT id, name FROM t WHERE id > ?")?;
//! let rows = stmt.query(&[Value::Int(0)])?;
...
/// 2. Use [`Client::query`], [`Client::execute`], [`Client::prepare`] for SQL operations
...
impl Client {
    ...
    pub fn connect(dsn: &str) -> Result<Self, Error> {
        ...
        Self::connect_with_dsn(&parsed)
    }

    /// Connect using a pre-parsed [`Dsn`].
    pub fn connect_with_dsn(dsn: &Dsn) -> Result<Self, Error> {
        ...
        conn.write_raw(&exchange)?;

        // Step 2: Read 4-byte redirect port
        let port_bytes = conn.read_exact_bytes(BROKER_RESPONSE_SIZE)?;
        let redirect_port =
        ...
        conn.write_raw(&open_db)?;

        // Step 5: Receive framed OpenDatabase response
        let response_data = conn.recv_framed()?;
    ...
    /// # use cubrid_protocol::value::Value;
    /// # let mut client = Client::connect("cubrid://dba:@localhost:33000/demodb")?;
    /// let affected = client.execute("INSERT INTO t (name) VALUES (?)", &[Value::from("hello")])?;
    ...
    pub fn execute(&mut self, sql: &str, params: &[Value]) -> Result<u64, Error> {
        ...
        if result.query_handle > 0 {
            self.close_query_handle(result.query_handle);
        }
    ...
    /// # use cubrid_protocol::value::Value;
    /// # let mut client = Client::connect("cubrid://dba:@localhost:33000/demodb")?;
    /// let rows = client.query("SELECT * FROM t WHERE id > ?", &[Value::Int(0)])?;
    /// for row in &rows {
    ///     println!("{:?}", row);
    /// }
    /// # Ok::<(), cubrid_client::Error>(())
    /// ```
    pub fn query(&mut self, sql: &str, params: &[Value]) -> Result<QueryResult, Error> {
        let auto_commit = self.auto_commit;
        let proto_version = self.proto_version;
        let conn = self.conn_mut()?;
        ...
        // Close handle
        if query_handle > 0 {
            self.close_query_handle(query_handle);
        }
    ...
    /// # use cubrid_protocol::value::Value;
    /// # let mut client = Client::connect("cubrid://dba:@localhost:33000/demodb")?;
    /// let mut stmt = client.prepare("SELECT * FROM t WHERE id = ?")?;
    /// let rows = stmt.query(&[Value::Int(1)])?;
    /// stmt.close(&mut client)?;
    /// # Ok::<(), cubrid_client::Error>(())
    /// ```
    pub fn prepare(&mut self, sql: &str) -> Result<Statement, Error> {
        let auto_commit = self.auto_commit;
        let conn = self.conn_mut()?;
        let cas_info = conn.cas_info();
    ...
    pub fn commit(&mut self) -> Result<(), Error> {
        self.end_tran(TransactionType::Commit)
    }

    /// Rollback the current transaction.
    pub fn rollback(&mut self) -> Result<(), Error> {
        self.end_tran(TransactionType::Rollback)
    }
    ...
    pub fn last_insert_id(&mut self) -> Result<String, Error> {
    ...
    pub fn close(&mut self) -> Result<(), Error> {
        if let Some(conn) = self.conn.take() {
            ...
            let _ = conn.write_and_close(&req);
        }
        Ok(())
    }
    ...
    fn conn_mut(&mut self) -> Result<&mut Connection, Error> {
        self.conn.as_mut().ok_or(Error::ConnectionClosed)
    }

    fn end_tran(&mut self, tran_type: TransactionType) -> Result<(), Error> {
        let conn = self.conn_mut()?;
        let cas_info = conn.cas_info();
        let req = handshake::write_end_tran(tran_type, &cas_info);
        conn.send_framed(&req)?;
        let resp = conn.recv_framed()?;
        // Parse simple response (CAS_INFO + response_code)
        let new_cas_info = parse_simple_response(&resp)?;
        conn.set_cas_info(new_cas_info);
        Ok(())
    }

    fn close_query_handle(&mut self, handle: i32) {
        if let Some(conn) = self.conn.as_mut() {
            let cas_info = conn.cas_info();
            let req = request::write_close_req_handle(handle, &cas_info);
            // Best-effort: ignore errors during cleanup
            if conn.send_framed(&req).is_ok() {
                if let Ok(resp) = conn.recv_framed() {
...
impl Statement {
    ...
    pub fn execute(&mut self, client: &mut Client, params: &[Value]) -> Result<u64, Error> {
        if self.closed {
            return Err(Error::ConnectionClosed);
        }
        if !params.is_empty() {
            // FC=41 fallback: interpolate params into SQL, use PrepareAndExecute
            return client.execute(&self.sql, params);
        }
    ...
    /// Query this prepared statement with the given parameters.
    ///
    /// Returns all rows.
    pub fn query(&mut self, params: &[Value]) -> Result<QueryResult, Error> {
        // Note: this method takes &mut self but needs client for I/O.
        // For simplicity, we return an error — use Statement::query_with(client, params) instead.
        let _ = params;
        Err(Error::Protocol(ProtocolError::InvalidPacket(
            "use Statement::query_with(client, params) instead".to_string(),
        )))
    }
    ...
    pub fn query_with(
        ...
        if !params.is_empty() {
            // FC=41 fallback: interpolate params into SQL, use PrepareAndExecute
            return client.query(&self.sql, params);
        }
        let auto_commit = client.auto_commit;
        let proto_version = client.proto_version;
        let conn = client.conn_mut()?;
    ...
    /// Get column metadata (available after prepare for SELECT statements).
    ...
    pub fn close(&mut self, client: &mut Client) -> Result<(), Error> {
        ...
        if self.query_handle > 0 {
            client.close_query_handle(self.query_handle);
            self.query_handle = 0;
        }
        Ok(())
    }
...
///
/// This matches the approach used by cubrid-go and pycubrid: replace `?` placeholders
/// with escaped literal values. This is necessary because CUBRID's CAS protocol
/// does not reliably support server-side bind parameters for all statement types.
...
mod tests {
    ...
    fn test_interpolate_bool() {
        let sql = "INSERT INTO t (flag) VALUES (?)";
        let result = interpolate_params(sql, &[Value::Bool(true)]);
        assert_eq!(result, "INSERT INTO t (flag) VALUES (1)");
    }

    #[test]
    fn test_interpolate_bytes() {
        let sql = "INSERT INTO t (data) VALUES (?)";
        let result = interpolate_params(sql, &[Value::Bytes(vec![0xCA, 0xFE])]);
        assert_eq!(result, "INSERT INTO t (data) VALUES (X'cafe')");
    }

    #[test]
    fn test_interpolate_date() {
        let sql = "INSERT INTO t (d) VALUES (?)";
        let result = interpolate_params(
            sql,
            &[Value::Date {
                year: 2024,
        ...
        assert_eq!(result, "INSERT INTO t (d) VALUES (DATE'2024-06-15')");
    }

    #[test]
    fn test_interpolate_time() {
        let sql = "INSERT INTO t (t) VALUES (?)";
        let result = interpolate_params(
            sql,
            &[Value::Time {
                hour: 14,
        ...
        assert_eq!(result, "INSERT INTO t (t) VALUES (TIME'14:30:45')");
    }

    #[test]
    fn test_interpolate_timestamp() {
        let sql = "INSERT INTO t (ts) VALUES (?)";
        let result = interpolate_params(
            sql,
            &[Value::Timestamp {
                year: 2024,
        ...
        assert_eq!(
            result,
            "INSERT INTO t (ts) VALUES (TIMESTAMP'2024-06-15 14:30:45')"
        );
    }

    #[test]
    fn test_interpolate_datetime() {
        let sql = "INSERT INTO t (dt) VALUES (?)";
        let result = interpolate_params(
            sql,
            &[Value::Datetime {
                year: 2024,
        ...
        assert_eq!(
            result,
            "INSERT INTO t (dt) VALUES (DATETIME'2024-06-15 14:30:45.123')"
        );
    }

    #[test]
    fn test_format_value_long() {
        assert_eq!(format_value(&Value::Long(9999999999)), "9999999999");
    }

    #[test]
    fn test_format_value_float() {
        let s = format_value(&Value::Float(1.5));
        assert!(s.starts_with("1.5"));
    }

    #[test]
    fn test_format_value_short() {
        assert_eq!(format_value(&Value::Short(42)), "42");
    }
    ...
    #[test]
    fn test_client_is_closed_initially_false() {
        // Can't test without a real connection, but we can test Error variants
